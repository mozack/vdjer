...
# print header
print '\t'.join(['sample', 'sequence', 'cdr3', 'expected_counts', 'seq_id', 'isotype', 'vregion_identity', 'aa_cdr3', 'vgene', 'jgene'])
...
for line in inv:
    ...
    # Load contigs
    sample_contigs = {}
    contig_file = results_dir + '/' + sample + '/' + chain_type + '/vdj_contigs.fa'
    cid = ''
    ...
    # Parse RSEM results
    rsem = results_dir + '/' + sample + '/' + chain_type + '/rsem_results.isoforms.results'
    rf = open(rsem, 'r')
    for rline in rf:
        rline = rline.strip()
        ...
        if not 'transcript' in rline:
            ...
            if float(ecount) >= 1:
                ...
                if vid[seq_id] != 'N/A' and vq_cdr3[seq_id] != 'N/A':
                    output = '\t'.join([sample, sequence, cdr3, ecount, seq_id, isotype, vid[seq_id], vq_cdr3[seq_id], vq_v[seq_id], vq_j[seq_id]])
